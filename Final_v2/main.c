...
GtkWidget *u_name, *pass, *resultat_assemblage, *resultat_recherche, *chercher;


void assembler_gtk (GtkWidget *assembler, gpointer data)
{
    char* sequence_final = assemblage();
    gtk_label_set_text(GTK_LABEL(resultat_assemblage),sequence_final);
}

void chercher_gtk (GtkWidget *rechercher_button, gpointer data)
{
    bool x;
    int n = taille_read("C:\\Users\\Msi\\Desktop\\genome.fq");
    char* motif  = malloc(2 * n * sizeof(char));
    motif = (char*)gtk_entry_get_text(GTK_ENTRY(chercher));
    x=rechercherFromReads(motif,"C:\\Users\\Msi\\Desktop\\genome.fq");
    if (x==true)
...
{
    GtkWidget *win, *label, *grid2, *assembler, *chercher_button, *Quit_button2;
    ...
    if (compare==0)
    {
        if (password==PASSWORD)
        {
            ...
            gtk_label_set_line_wrap(GTK_LABEL(resultat_assemblage),TRUE);
            gtk_widget_set_size_request(GTK_WIDGET(resultat_assemblage),100,-1);
